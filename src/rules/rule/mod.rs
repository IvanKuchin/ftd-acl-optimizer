...
mod network_object;
use network_object::NetworkObjectItem;
...
impl TryFrom <Vec<String>> for Rule {
    ...
    fn try_from(lines: Vec<String>) -> Result<Self, Self::Error> {
        ...
        let _source_networks: Vec<_> = lines_from_till(&lines, "Source Networks", &["Destination Networks", "Source Ports", "Destination Ports", "Logging"])?;
        let _destination_networks: Vec<_> = lines_from_till(&lines, "Destination Networks", &["Source Networks", "Source Ports", "Destination Ports", "Logging"])?;

        Ok(Self{ name})
    }
}

fn get_name(lines: &Vec<String>) -> Result<String, RuleError> {
    let line = lines.iter().find(|line| line.contains("Rule: ")).ok_or(RuleError::General("Rule name not found".to_string()))?;
    let name = line.split_whitespace().nth(2).ok_or(RuleError::General("Rule name not found".to_string()))?;
    Ok(name.to_string())
}

fn lines_from_till(lines: &Vec<String>, start: &str, end: &[&str]) -> Result<Vec<String>, RuleError> {
...
mod tests {
    ...
    fn test_lines_from_till_with_multiple_end_markers() {
        ...
        let result = lines_from_till(&lines, "Source Networks", &["Destination Networks", "Source Ports"]).unwrap();
        assert_eq!(result, vec![
            "Source Networks       : Internal (group)".to_string(),
            "OBJ-192.168.0.0 (192.168.0.0/16)".to_string(),
        ]);
